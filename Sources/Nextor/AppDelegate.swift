...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    private func setupStatusItem() {
        ...
        if let button = item.button {
            button.image = NSImage(systemSymbolName: "calendar.badge.plus", accessibilityDescription: "Nextor")
        ...
        let about = NSMenuItem(title: L("About Nextor", "关于 Nextor", "Nextor について"), action: #selector(showAbout), keyEquivalent: "")
        about.target = self
        menu.addItem(about)
        menu.addItem(.separator())

        let quit = NSMenuItem(title: L("Quit Nextor", "退出 Nextor", "Nextor を終了"), action: #selector(quit), keyEquivalent: "q")
        quit.target = self
        menu.addItem(quit)
