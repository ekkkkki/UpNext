...
[![CI](https://github.com/ekkkkki/Nextor/actions/workflows/ci.yml/badge.svg)](https://github.com/ekkkkki/Nextor/actions/workflows/ci.yml)
...
<img src="docs/shots/upcoming-en.png" width="760" alt="Nextor opens to your agenda — overdue, today and the next few days, grouped by day">

</div>

Press **⇧⌘A** anywhere, type the way you think — `明天下午3点 开会 30min`, `Team sync tomorrow 2-3pm ~Work`,
or just paste a whole address — and Nextor decides whether it's a **Reminder** or a **Calendar event**,
...
Type the way you think — Nextor decides reminder vs. event and pulls out the time, place and recurrence:

<img src="docs/shots/hero.png" width="720" alt="Nextor turning a typed line into a calendar event with the location filled in">
...
This builds a release binary, assembles `dist/Nextor.app`, embeds an icon, ad-hoc code-signs it,
and produces `dist/Nextor.dmg`. Then:

```bash
open dist            # drag Nextor.app to /Applications
```
...
Open `Nextor.dmg`, drag **Nextor** to **Applications**. Because the app is ad-hoc signed
...
On first launch Nextor shows a short welcome and asks for **Reminders** and **Calendar** access.
...
<img src="docs/shots/onboarding.png" width="420" alt="Nextor first-run welcome">
...
```bash
swift run NextorTests          # unit tests: parsing, dates, recurrence, location, search query (118 checks)
.build/debug/Nextor --smoke-test          # launch / run-loop boot check
.build/debug/Nextor --selftest-ui         # headless UI/layout test (catches input clipping/growth)
Nextor --selftest-eventkit                # end-to-end: create → search → delete real items + location round-trip
```

`--selftest-eventkit` needs Reminders/Calendar access; it creates two clearly-labeled
`Nextor self-test ✓` items, verifies search finds them, then deletes them.
...
```
Sources/
  NextorCore/        Pure logic, no UI/EventKit — fully unit-tested
  ...
  NextorTests/       Self-contained assertion runner (no XCTest dependency)
...
- **Runs on macOS 14+.** Building from source needs the **macOS 26 SDK** (the optional Apple
  ...
  CI builds and tests only `NextorCore`, which has no such requirement.
